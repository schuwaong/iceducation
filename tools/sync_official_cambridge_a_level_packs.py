...
DEFAULT_DATA_DIR = ROOT / "data" / "cambridge-a-level-syllabuses"
DEFAULT_OUTPUT_PATH = ROOT / "generated-cambridge-a-level-packs.js"
...
SYLLABUS_NAME = "Cambridge A Level"
LEVELS = ["AS Level", "A Level"]

DATA_BEGIN = "// BEGIN_IC_EDUCATE_OFFICIAL_A_LEVEL_PACK_DATA"
DATA_END = "// END_IC_EDUCATE_OFFICIAL_A_LEVEL_PACK_DATA"
...
def parse_official_subjects(html_text: str) -> list[OfficialSubject]:
    ...
    for match in pattern.finditer(html_text):
        ...
        if not code_match:
            continue
        if "/programmes-and-qualifications/cambridge-international-as-and-a-level" not in href:
            continue
...
def parse_snapshot_subject_labels(path: Path) -> set[str]:
    ...
    a_level = next((item for item in syllabi if item.get("name") == SYLLABUS_NAME), None)
    if not a_level:
        return set()
    return {safe_text(subject.get("label") or subject.get("value")) for subject in a_level.get("subjects") or []}
...
def extract_focus_items(text: str, subject_label: str) -> list[str]:
    ...
    for line in lines:
        if not line or len(line) < 5 or len(line) > 96:
            continue
        lower = line.lower()
        if any(skip in lower for skip in ["cambridge international as & a level", "cambridge international as and a level", "syllabus", "www.", "copyright", "back to contents"]):
            continue
...
def build_pack(subject: OfficialSubject, pdf_url: str, focus_items: list[str]) -> dict[str, Any]:
    ...
    pack_id = f"official-cambridge-a-level-{subject.code}"
    selected = focus_items[:6] if focus_items else DEFAULT_FOCUS[:6]
    note_cards = [
        {
            "title": "Syllabus map",
            "points": [
                f"This pack is built from the official Cambridge International AS & A Level {subject_name} {subject.code} syllabus PDF.",
    ...
    return {
        ...
        "subtitle": f"Cambridge International AS & A Level {subject.code} - official 2026 syllabus overview",
        "officialALevel": {
            "code": subject.code,
        ...
        "notes": {
            "focusItems": selected,
            "importantPoints": [
                f"Use the official Cambridge International AS & A Level {subject_name} syllabus as the coverage checklist.",
        ...
        "worksheet": {
            "intro": f"Printable overview practice for Cambridge International AS & A Level {subject_name} {subject.code}.",
...
{DATA_END}
  ...
  function requestSubjectCode(request) {{
    if (safeText(request?.syllabus) !== "Cambridge A Level") {{
      return "";
    }}
  ...
  window.IC_EDUCATE_OFFICIAL_A_LEVEL_SUBJECT_PACKS = {{
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Download official Cambridge International AS & A Level syllabuses and generate subject packs.")
...
def main() -> int:
    ...
    print(f"Official Cambridge AS & A Level entries found: {len(official_subjects)}")
    print(f"Current snapshot Cambridge A Level subjects: {', '.join(sorted(local_labels))}")
    print(f"Official A Level entries selected for generated bundle: {len(missing)}")
    ...
    for index, subject in enumerate(queue, start=1):
        ...
        try:
            ...
            data["metadata"] = {
                "generatedAt": utc_now(),
                "source": "Cambridge International official AS & A Level subjects page",
    ...
    print(f"Wrote {len(packs)} official Cambridge AS & A Level subject pack(s) to {args.output}")
    if failures:
        print("Failures:")
